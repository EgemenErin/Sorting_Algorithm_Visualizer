...
def merge_sort(draw_info, ascending=True):
    ...
    yield from merge_sort_helper(draw_info, left_half, ascending, 0)
    yield from merge_sort_helper(draw_info, right_half, ascending, mid)
...
def merge_sort_helper(draw_info, lst, ascending, offset):
    if len(lst) <= 1:
        return lst
    ...
    yield from merge_sort_helper(draw_info, left_half, ascending, offset)
    yield from merge_sort_helper(draw_info, right_half, ascending, offset + mid)
    ...
    while i < len(left_half) and j < len(right_half):
        if (left_half[i] < right_half[j] and ascending) or (left_half[i] > right_half[j] and not ascending):
            ...
            j += 1
        k += 1
        draw_list(draw_info, {offset + k - 1: draw_info.GREEN}, True)
        yield True
